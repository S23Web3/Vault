...
**Still needed before locking:** Add two sections to `C:\Users\User\Documents\Obsidian Vault\PROJECTS\four-pillars-backtester\docs\VINCE-V2-CONCEPT-v2.md`:

1. **GUI section** -- Dash framework, panel layout, interaction model
2. **Architecture skeleton** -- API layer, types, separation rule (GUI calls API, API calls engine)

Then change status to `APPROVED FOR BUILD`.
