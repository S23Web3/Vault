...
1. **Auth**: BingX uses HMAC-SHA256 query-param signing. WEEX build spec shows
   header-based: `X-API-KEY`, `X-TIMESTAMP`, `X-SIGN`. Exact scheme TBD.
2. **Symbol format**: BingX = `BTC-USDT`. WEEX contract API = `cmt_btcusdt` (probe confirmed).
   Order API format may differ — needs verification.
3. **Position mode**: BingX requires explicit LONG/SHORT positionSide (hedge mode).
   WEEX equivalent needs verification.
...
connector:
  ...
  demo_mode: false  # toggle when testnet available
coins:
  - BTCUSDT   # WEEX symbol format (TBD)
strategy:
...
- Write `docs/WEEX-API-REFERENCE.md` (like `BINGX-API-V3-REFERENCE.md`)
...
- `tests/` — 67 tests equivalent (auth, risk gate, state, executor, integration)
