...
**Visualizer plan (S9):**
- BBWP line with spectrum coloring (blue to green to yellow to red)
- MA line (white)
...
- SL line changing style: Red solid (P0) to dotted (P1) to dashed (P2) to yellow trail (P3)
- TP line: solid (P0) to dotted (P1) to dashed (P2) to removed (P3)
...
Add build_s6() through build_s11() to the existing v1 file (607 lines, would grow to ~1200).

**Option B — New versioned file (recommended):**
Write a NEW file:
`C:\Users\User\Documents\Obsidian Vault\PROJECTS\four-pillars-backtester\scripts\visualize_strategy_perspectives_v2.py`

Contains ALL 11 strategies (S1–S5 ported + S6–S11 new). Cleaner, keeps v1 intact.

**Recommendation: Option B.** Reasons:
- Keeps v1 untouched (already verified by user)
...
results/strategy_perspectives/
  S1_bible.png
  S2_rebate_farming.png
  S3_v38_directional.png
  S4_v384_current.png
  S5_5589_scalp.png
...
2. `python scripts/visualize_strategy_perspectives_v2.py` — must produce 11 PNGs
3. User visually checks each PNG: do indicator panels match the described perspectives?
