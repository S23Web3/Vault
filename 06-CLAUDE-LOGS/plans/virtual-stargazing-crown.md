...
One build script: `C:\Users\User\Documents\Obsidian Vault\PROJECTS\bingx-connector\scripts\build_mechanical_audit_fixes.py`

Creates `.bak` backups of all modified files before patching. Runs `py_compile` on each. Reports PASS/FAIL for every patch.

---

## Verification

1. `py_compile` all 5 modified files
2. Review trades.csv after next few trades -- TTP exits should show real fill price, not trail_level
3. BE-raised trades should all show small positive PnL (buffer protecting against slippage)
4. Ultra-volatile coins (Q-USDT class, atr_ratio > 1.5%) should be blocked by risk gate
5. Bot restart required to activate all fixes
6. Run command: `cd "C:\Users\User\Documents\Obsidian Vault\PROJECTS\bingx-connector" && python main.py`
