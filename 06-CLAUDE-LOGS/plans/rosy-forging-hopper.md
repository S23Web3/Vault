...
## Critical Context: S4 Was Rebate-Subsidised, Not Self-Sustaining

**User-stated fact**: "384 only made sense because rebate was pumped back into the system."

This reframes the entire v4 problem. The bot was never independently profitable — it was viable because rebate income covered trading losses. When the rebate environment changed (WEEX -> BingX, different tier, different margin size), the math broke.

### What "Rebate Covers Losses" Requires

At BingX current config ($5 margin, 10x, $50 notional per trade):

- Round-trip commission = $50 × 0.0016 = **$0.08 per trade**
- If rebate = 70%: rebate per trade = $0.08 × 0.70 = **$0.056**
- Net commission cost after rebate = $0.08 × 0.30 = **$0.024 per trade**

For rebates to cover ALL trading losses (v4 NORTH STAR baseline):

```
required: rebate_per_trade × N_trades >= avg_loss × N_losing_trades
=> $0.056 × N >= avg_loss × (1 - WR) × N
=> avg_loss <= $0.056 / (1 - WR)

At WR=50%: avg_loss must be <= $0.112  (11.2 cents max per losing trade at $50 notional)
At WR=45%: avg_loss must be <= $0.102
```

Current bot: avg_loss = $0.44. That is 4x the maximum loss the rebate can cover at $50 notional with 70% rebate at 50% WR. The rebate model is structurally broken at current SL distances.

### Why This Wasn't a Problem on WEEX

WEEX was $500 margin × 20x = $10,000 notional. Rebate per trade = $10,000 × 0.0016 × 0.70 = $11.20. At that scale, an avg_loss of $0.44 is noise. The rebate swamped the loss. On BingX at $50 notional, the rebate is $0.056 — the loss is 8x the rebate.

**Conclusion**: The rebate pump only works at high notional. At $50 notional on BingX, v4 must be independently breakeven (win rate × avg win >= loss rate × avg loss). Rebates become a bonus, not the survival mechanism.

### v4 Breakeven Target (Non-Negotiable Design Constraint)

```
Required for independent BE: R:R × WR >= (1 - WR)
=> R:R >= (1 - WR) / WR
...
Current R:R = 0.28 at unknown WR. Even if WR = 55%, need R:R >= 0.82. Current is 0.28 — 3x short.

This target is the hard constraint for any v4 design. If the backtest doesn't hit R:R > 1.0 (assuming ~50% WR), don't deploy.
...
|----------|---|---|---|-------|-------|
| S1 (Bible / original Four Pillars) | ✓ | ✓ partial | ✓ partial | **75%** | Best alignment |
| S2 (Rebate volume farming) | ✗ | ✗ | ✗ | **0%** | Different paradigm entirely |
| S3 (v3.8 simple crossover) | ✓ | ✗ | ✗ | **20%** | Has A only |
| S4 (v3.8.4 current — simultaneous) | ✓ | ✗ VIOLATED | ✗ | **10%** | Pile-in violates B; code contradicts its own description |
| S5 (55/89 EMA scalp) | ✓ | ✓ partial | ✗ | **35%** | Macro gate partial B, no divergence |
| S6 (Ripster cloud structural) | ✗ | ✗ | ✗ | **5%** | Cloud structure only, no stochastics |
| S7 (Quad Rotation Stochastic) | ✓ | ✓ | ✓ | **70%** | Highest divergence alignment |
| S8 (Three Pillars / Core-Strategy v2) | ✓ | ✓ | ✗ | **45%** | Correct trigger (Stoch 55), no divergence |
| S9 (BBWP volatility filter) | ✗ | ✗ | ✗ | **0%** | Filter only — correct role |
| S10 (ATR Phase exit) | N/A | N/A | N/A | N/A | Exit-side management, not entry |
| S11 (AVWAP structural) | ✗ | ✗ | ✗ | **15%** | Volume context only |
...
- **A** ✓ — Stoch 9 < 20 explicitly required
- **B** ✓ partial — Stoch 60 > 80 is a macro condition (it's IN the uptrend, not cascading out of oversold). Stoch 40 divergence check is post-entry hold logic, not pre-entry cascade gate.
- **C** ✓ partial — "Stoch 40 makes higher lows" IS a divergence element. But it is framed as a hold condition, not a pre-entry filter. The divergence structure is implied, not required to fire the signal.
...
- **A** ✓ — stoch_9 < 25 entry zone
- **B** ✗ — signal fires on stoch_9 crossing alone, no cascade confirmation
- **C** ✗ — no divergence, no channel
...
- **B** ✗ VIOLATED — simultaneous pile-in is the anti-pattern of cascade
...
- **A** ✓ — Stoch 9 oversold trigger in overzone design
- **B** ✓ partial — stoch_40 > 50 AND stoch_60 > 50 as GATE condition. These are positioning gates (stochs already above midline = macro healthy), not cascade exits from extreme. This validates uptrend continuation context but doesn't detect the cascade itself.
- **C** ✗ — AVWAP -2 sigma trail is exit-side. No divergence detection on entry.

Better than S4 (no pile-in), but macro gate ≠ cascade exit. The gate prevents entries in macro downtrends. Still misses the "other stochs move with or soon after" element.
...
Source: `02-STRATEGY/Indicators/Quad-Rotation-Stochastic.md`
...
- **A** ✓ — oversold filter (< 20) on both current and previous pivot
- **B** ✓ — has stoch_40 and stoch_60 in the rotation concept (though pivot method focuses on stoch_9 divergence)
- **C** ✓ — explicit bullish divergence with oversold filter
...
Source: `02-STRATEGY/Core-Trading-Strategy.md` (v2, 2026-01-29)

This is the ONLY strategy that implements the correct trigger as documented by Malik: **Stoch 55 crosses UP**.

- **A** ✓ — Stoch 9 < 20 explicitly as staging condition
- **B** ✓ — Stoch 55 cross is the entry signal. ATR climbing + BBWP transitioning are continuation checks.
- **C** ✗ — No explicit pre-entry divergence requirement. Momentum continuation check is post-entry ("if momentum declines, exit"). Pre-entry divergence is not in the spec.
...
def detect_bullish_divergence(closes, stoch_9, left=3, right=3, os_level=20):
    ...
    Returns (divergence_at_bar, confidence)
    """
...
def compute_regression_channel(closes, lookback=20):
    ...
    if var == 0 or ym == 0:
        return 0.0, 0.0, 0.0
    slope = cov / var
    slope_pct = slope / ym           # normalized: % per bar
...
- `reg_r2_min` (default 0.55): minimum R² for "orderly" channel
