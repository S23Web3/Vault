...
def fetch_symbol_full(session, symbol, start_ms, end_ms, coin_bar=None, coin_base=0.0):
    ...
    total_candles = 0
    prev_outer_n = coin_base  # track last outer bar value for this coin
    ...
    while cursor < end_ms:
        ...
        if not raw:
            empty_streak += 1
            # Jump forward faster through empty ranges (no data = not listed yet)
            if empty_streak >= 3:
    ...
    pbar.refresh()
    pbar.close()
    # Snap outer bar to full coin completion
    _update_outer(100)
...
def main():
    ...
    try:
        for idx, sym in enumerate(symbols):
            ...
            # coin_base = how many full coins completed so far (fractional outer bar)
            coin_base = float(idx)

            if args.skip_existing and parq_1m.exists():
                skipped += 1
                # snap outer bar to next whole coin
            ...
            success += 1
            # outer bar already advanced fractionally; snap to exact whole coin
            coin_bar.n = float(idx + 1)
            coin_bar.refresh()
