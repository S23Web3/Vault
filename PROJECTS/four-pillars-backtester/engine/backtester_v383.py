"""
v3.8.3 backtest engine: ATR/AVWAP SL, scale-out, D signal, AVWAP inheritance.

Per-bar execution order:
1. Commission settlement check
2. Check exits on all slots (SL from previous bar)
...
from .position_v383 import PositionSlot383, Trade383
from .commission import CommissionModel
from .avwap import AVWAPTracker


class Backtester383:
    """v3.8.3 multi-slot backtester with scale-out mechanism."""
    ...
    def run(self, df: pd.DataFrame) -> dict:
        """Run v3.8.3 backtest on signal-enriched DataFrame."""
        ...
        # Cloud 3 filters (for B/C/R gating only, not SL)
        ...
        # State
        slots: list[Optional[PositionSlot383]] = [None] * 4
        last_entry_bar: Optional[int] = None
        trades: list[Trade383] = []
        equity = self.initial_equity
        equity_curve = np.full(n, equity)
        position_counts = np.zeros(n, dtype=int)  # active slots per bar
        ...
        # Re-entry state (full AVWAP clone from stopped-out slot)
        ...
        for i in range(n):
            ...
            for s in range(4):
                ...
                if exit_reason:
                    comm_exit = self.comm.charge_custom(slots[s].notional)
            ...
            for s in range(4):
                ...
                if slots[s].check_scale_out(i, close[i]):
                    ...
                    comm_exit = self.comm.charge_custom(scale_notional)
                    # Final scale-out gets entry_commission for accurate metrics
                    trade_comm = comm_exit + (slots[s].entry_commission if is_final_check else 0)
            ...
            # A/D: no Cloud 3 gate; B/C/R: Cloud 3 gated
            ...
            # D long (no Cloud 3 gate, same as A)
            if not did_enter and long_d[i] and can_long_a:
                empty = self._find_empty(slots)
                if empty >= 0:
                    did_enter = True
                    self._open_slot(slots, empty, "LONG", "D", i,
                                    close[i], atr[i], hlc3[i], vol[i])
                    comm_entry = self.comm.charge()
                    equity -= comm_entry
                    slots[empty].entry_commission = comm_entry
            ...
            if not did_enter and short_d[i] and can_short_a:
                ...
                if empty >= 0:
                    did_enter = True
                    self._open_slot(slots, empty, "SHORT", "D", i,
                                    close[i], atr[i], hlc3[i], vol[i])
                    comm_entry = self.comm.charge()
                    equity -= comm_entry
                    slots[empty].entry_commission = comm_entry
            ...
            # --- Step 6: AVWAP adds (limit orders with parent AVWAP inheritance) ---
            active_count = sum(1 for s in slots if s is not None)
            ...
            # --- Step 7: AVWAP re-entry (limit orders with AVWAP inheritance) ---
            active_count = sum(1 for s in slots if s is not None)
            ...
            # Track position count at end of bar
        ...
        # Capital utilization
        margin_per_pos = self.notional / 20.0  # 20x leverage
    ...
    def _compute_metrics(self, trades, equity_curve):
        """Compute metrics compatible with existing format."""
        if not trades:
