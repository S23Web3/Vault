...
class Backtester:
    ...
    def __init__(self, params: dict = None):
        ...
        # AVWAP trailing stop
        self.avwap_enabled = p.get("avwap_enabled", False)
        self.avwap_band = p.get("avwap_band", 1)
        self.avwap_floor_atr = p.get("avwap_floor_atr", 1.0)

        # Commission -- rate-based: notional * commission_rate = cost per side
    ...
    def run(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(n):
            ...
            if self.position is not None:
                exit_reason = self.position.update(
                    high[i], low[i], close[i],
                    hlc3=hlc3[i], volume=vol[i]
                )
            ...
            if enter_dir is not None:
                ...
                self.position = Position(
                    ...
                    avwap_enabled=self.avwap_enabled,
                    avwap_band=self.avwap_band,
                    avwap_floor_atr=self.avwap_floor_atr,
                )
                self.entry_bar = i
