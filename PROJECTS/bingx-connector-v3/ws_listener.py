...
class WSListener(threading.Thread):
    ...
    async def _ws_loop(self):
        ...
        if reconnect_count >= MAX_RECONNECT:
            ...
            # W14: Use datetime.now(timezone.utc) instead of deprecated utcnow()
            _ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
            # W15: Use absolute path relative to this file, not relative Path("logs")
            _flag_dir = Path(__file__).resolve().parent / "logs"
