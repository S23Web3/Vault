...
def get_time_sync(base_url=None, sync_interval=30):
    ...
    W09: If singleton already exists and base_url differs, log a WARNING.
    """
    global _instance
    with _lock:
        if _instance is None:
            ...
            resolved = DEMO_SERVER_TIME_URL if "vst" in base_url else LIVE_SERVER_TIME_URL
            if resolved != _instance._server_time_url:
                logger.warning(
                    "TimeSync singleton already exists with URL %s, "
                    "ignoring new base_url that resolves to %s",
                    _instance._server_time_url, resolved)
        return _instance
