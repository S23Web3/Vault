"""
BingX Connector v3 main entry point.
...
def setup_logging():
    """Configure dual logging: file + console with UTC+4 timestamps. W11: Cleans up old log files."""
    log_dir = Path(__file__).resolve().parent / "logs"
    log_dir.mkdir(exist_ok=True)
    today = date.today().strftime("%Y-%m-%d")
    log_file = log_dir / (today + "-bot.log")

    # W11: Clean up log files older than 30 days
    cutoff = datetime.now(timezone.utc) - timedelta(days=30)
    for old_file in log_dir.glob("*-bot.log"):
        try:
            mtime = datetime.fromtimestamp(old_file.stat().st_mtime, tz=timezone.utc)
            if mtime < cutoff:
                old_file.unlink()
                # Can't use logger yet (not configured), so print
                print("Deleted old log: " + str(old_file))
        except OSError:
            pass
...
def fetch_commission_rate(auth):
    """Fetch taker commission rate from BingX. Returns round-trip rate (float) or default 0.0016.

    W07: Return value is commission_rate_rt -- Round-trip (entry + exit) taker fee.
    """
...
def main():
    ...
    setup_logging()
    logger.info("=== BingX Connector v3 Starting ===")
    ...
    commission_rate_rt = fetch_commission_rate(auth)
    ...
    monitor = PositionMonitor(
        ...
        commission_rate_rt=commission_rate_rt,
    ...
    try:
        ...
        if data.get("code", 0) != 0:
            ...
            # W12: Pass notifier and auth so reconcile can alert and look up fill prices
            state_mgr.reconcile(live_pos, notifier=notifier, auth=auth)
    except Exception as e:
    ...
    logger.info("=== BingX Connector v3 Stopped ===")
