...
class Executor:
    ...
    def __init__(self, auth, state_manager, notifier, position_config):
        ...
        # W06: Contracts cache to avoid fetching ALL contracts every call
    ...
    def _raw_post(self, url, headers=None):
        """Execute POST. Returns full response dict (caller must check code) or None on network error."""
        try:
    ...
    def fetch_step_size(self, symbol):
        """Fetch step size for a symbol. W06: Uses 1-hour cache. Returns float or None."""
        now = time.time()
        if self._contracts_cache and (now - self._cache_ts) < self._cache_ttl:
            # Use cached contracts
            step = self._contracts_cache.get(symbol)
            if step is not None:
                return step
            logger.error("Symbol %s not found in cached contracts", symbol)
            return None
        ...
        try:
            ...
            for c in contracts:
                sym = c.get("symbol", "")
                step_val = float(c.get("tradeMinQuantity",
                                       c.get("stepSize", 1)))
                self._contracts_cache[sym] = step_val
            self._cache_ts = now
            step = self._contracts_cache.get(symbol)
            if step is not None:
                return step
    ...
    def _place_trailing_order(self, symbol, direction, quantity, activation_price, price_rate=None):
        ...
        result = self._raw_post(req["url"], headers=req["headers"])
        if result is None:
    ...
    def execute(self, signal, symbol):
        ...
        if error_code != 0:
            if error_code == 101209:
                # W04: Max position value exceeded -- retry with halved qty using fresh params copy
                ...
                if halved_qty > 0:
                    ...
                    retry_params["quantity"] = str(halved_qty)
                    req2 = self.auth.build_signed_request(
                        "POST", ORDER_PATH, retry_params)
