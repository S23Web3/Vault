...
def load_trades() -> pd.DataFrame:
    ...
    try:
        import csv as _csv
        LOG.debug("load_trades: reading %s (exists=%s)", TRADES_PATH, TRADES_PATH.exists())
        rows = []
        with open(TRADES_PATH, 'r', encoding='utf-8', newline='') as f:
            ...
            for row in reader:
                if len(row) >= n:
                    rows.append(row[:n])
        LOG.debug("load_trades: header=%d cols, rows=%d", n, len(rows))
        if not rows:
            return pd.DataFrame()
...
app.layout = html.Div([
    ...
    dcc.Store(id='store-bot-status', storage_type='memory'),
...
def update_status_bar(state_json, config_json, unrealized_json, _n_intervals):
    ...
    try:
        ...
        # Detect stale session -- if session_start is not today, counters are stale
