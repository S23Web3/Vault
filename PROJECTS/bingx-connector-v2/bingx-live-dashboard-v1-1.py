...
def compute_metrics(df: pd.DataFrame) -> dict:
    """Compute summary performance metrics from closed trades DataFrame. Returns dict."""
    # Guard against empty or missing data
    if df.empty or "pnl_net" not in df.columns:
        return {"total": 0, "win_rate": 0.0, "profit_factor": 0.0, "avg_win": 0.0,
                "avg_loss": 0.0, "expectancy": 0.0, "max_dd": 0.0, "gross_pnl": 0.0}
    ...
    max_dd = round(float((cum - cum.cummax()).min()), 4) if len(cum) > 0 else 0.0
    ...
    return {
        ...
        "max_dd": max_dd,
        "gross_pnl": round(gross_wins - gross_losses, 4),
    }
...
def make_history_tab() -> html.Div:
    ...
    return html.Div([
        html.Div([
            dcc.Checklist(id='hist-today-filter',
...
app = dash.Dash(
    __name__,
    suppress_callback_exceptions=True,  # Required — tab IDs don't exist at startup
    title='BingX Live Dashboard v1-1',
)
...
def load_all_data(n_intervals):
    ...
    try:
        ...
        LOG.debug("Data loaded: %d positions, %d trades",
...
@callback(
    ...
    prevent_initial_call=False,  # Must render default tab on load
)
def render_tab(tab_value):
    """Return layout for the selected tab."""
    if tab_value == 'tab-ops':
        return make_operational_tab()
    if tab_value == 'tab-hist':
        return make_history_tab()
    if tab_value == 'tab-analytics':
        return make_analytics_tab()
    if tab_value == 'tab-coins':
        return make_coin_summary_tab()
    if tab_value == 'tab-controls':
        return make_bot_controls_tab()
...
def update_status_bar(state_json, config_json):
    ...
    try:
        ...
        return html.Div([
            ...
            build_metric_card("Daily PnL", f"${daily_pnl:+.2f}", pnl_color),
...
@callback(
    ...
    Input('store-trades', 'data'),  # Input not State — must refresh when data reloads
    prevent_initial_call=True,  # Do not fire on page load — stores not yet populated
)
def update_history_grid(today_filter, dir_filter, grade_filter, exit_filter, trades_json):
    ...
    try:
        ...
        summary = html.Span(
            f"{len(df)} trades | Total PnL: {total_pnl:+.4f}",
...
def update_analytics(trades_json):
    ...
    try:
        ...
        cards = html.Div([
            ...
            build_metric_card("Win Rate", f"{metrics['win_rate']}%",
            ...
            build_metric_card("Expectancy", f"${metrics['expectancy']:+.4f}",
                              COLORS['green'] if metrics['expectancy'] >= 0 else COLORS['red']),
            build_metric_card("Avg Win", f"${metrics['avg_win']:+.4f}", COLORS['green']),
            build_metric_card("Avg Loss", f"${metrics['avg_loss']:+.4f}", COLORS['red']),
            build_metric_card("Max DD", f"${metrics['max_dd']:.4f}", COLORS['red']),
        ], style={'display': 'flex', 'gap': '12px', 'flexWrap': 'wrap'})
        ...
        return (html.Div(f"Error: {e}", style={'color': COLORS['red']}),
                ...
                html.Div(f"Error: {e}", style={'color': COLORS['red']}))
