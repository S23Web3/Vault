#!/usr/bin/env python3
"""
Trade Chart Report v2 - Interactive HTML with Plotly charts.
Paginated one-trade-per-page, AVWAP SD bands, toggle overlays,
structured @anchor notes, entry/exit markers.

Run: python scripts/run_trade_chart_report_v2.py --date 2026-03-11
"""
import argparse
import csv
import hashlib
...
def compute_indicators(df, entry_idx):
    ...
    rsi = rsi.fillna(50.0)
    df["tdi_rsi"] = rsi.values
    # Volatility bands on RSI
    rsi_sma = rsi.rolling(band_length, min_periods=1).mean()
    rsi_std = rsi.rolling(band_length, min_periods=1).std()
    df["tdi_upper"] = (rsi_sma + 1.6185 * rsi_std).values
    df["tdi_lower"] = (rsi_sma - 1.6185 * rsi_std).values
    df["tdi_mid"] = ((df["tdi_upper"] + df["tdi_lower"]) / 2.0)
    # Fast MA on RSI (RSI Price Line -- green)
    df["tdi_fast"] = rsi.rolling(fast_ma_len, min_periods=1).mean().values
    # Slow MA on RSI (Trade Signal Line -- red)
    df["tdi_slow"] = rsi.rolling(slow_ma_len, min_periods=1).mean().values
...
def build_trade_chart(trade, kline_df, trade_num, has_signals):
    ...
    fig = make_subplots(
        ...
        subplot_titles=["", "Stoch 9", "Stoch 14", "TDI 14", "Stoch 40", "Stoch 60"],
    )
...
def build_html_report(trades_info, charts_html, target_date, from_time):
    ...
    p.append('<label><input type="checkbox" id="tog-tdi" checked onchange="toggleOverlay(\'tdi\')"> TDI 14</label>')
