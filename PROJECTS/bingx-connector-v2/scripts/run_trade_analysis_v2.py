...
def load_trades(from_date, to_date):
    ...
    # DateTime filter using entry_time (preferred) or timestamp (fallback)
    # from_date/to_date can be YYYY-MM-DD or YYYY-MM-DDTHH:MM (ISO prefix)
    def get_ts_str(row):
        """Extract full ISO timestamp string from entry_time or timestamp."""
        et = row.get("entry_time", "")
        ts = row.get("timestamp", "")
        val = et if (et and not (isinstance(et, float) and pd.isna(et))) else ts
        return str(val)[:25] if val else ""

    df["_ts_key"] = df.apply(get_ts_str, axis=1)
    # Pad short filters: "2026-03-04" -> "2026-03-04T00:00"
    from_cmp = from_date if "T" in from_date else from_date + "T00:00"
    to_cmp = to_date + "T23:59:59" if "T" not in to_date else to_date
    mask = (df["_ts_key"] >= from_cmp) & (df["_ts_key"] <= to_cmp)
    df = df[mask].copy()
    df.drop(columns=["_ts_key"], inplace=True)
...
def main():
    ...
    parser.add_argument("--from", dest="from_date", default="2026-03-04T16:00",
                        help="Start YYYY-MM-DD or YYYY-MM-DDTHH:MM (default: 2026-03-04T16:00 = 8pm Dubai)")
