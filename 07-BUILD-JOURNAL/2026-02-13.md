...
## Session 3: Operational Logic Audit -- 7 Critical Fixes Applied

**Started**: 2026-02-14 (continued from 2026-02-13 session)

### Audit Summary

3 parallel agents audited backtester engine, data pipeline, and ML pipeline. Found 7 CRITICAL, 15 WARNING, 17 OK. Full report: `06-CLAUDE-LOGS/2026-02-14-operational-logic-audit.md`

### Fixes Applied

| ID | File | Fix | Impact |
|----|------|-----|--------|
| C-4 | `ml/features.py:163`, `ml/features_v2.py:326` | Removed `duration_bars` from `get_feature_columns()` | Eliminates look-ahead bias (uses exit_bar which is future info) |
| C-5 | `ml/features_v2.py:113` | Added `daily_sum.shift(1)` before rolling MA | Prevents using today's volume total at intraday bars |
| C-3 | `engine/commission.py:20-32` | Removed `cost_per_side` parameter backdoor | Forces rate-based commission (0.0008 * notional), no override |
| C-7 | `scripts/dashboard.py:883,898,1024,1044` | Switched ML training from `label_trades` to `label_trades_by_pnl` | Labels now account for commission cost (net P&L, not exit type) |
| C-2 | `engine/backtester_v384.py:175` | Prorated entry commission on scale-outs | `entry_comm * scale_notional / original_notional` instead of all-or-nothing |
| C-1 | `engine/backtester_v384.py:393` | Added unrealized P&L to equity curve | Mark-to-market: `equity + sum(unrealized from open slots)` |
| C-6 | `scripts/dashboard.py:1049-1080` | Wired purged CV into walk-forward training | IS accuracy from purged k-fold (not train accuracy), honest WFE |

### Verification Commands

```
python scripts/dashboard.py
# Run single backtest, check Tab 4 (ML) and Tab 5 (Validation)
# Verify equity curve shows drawdowns during open positions
# Verify WFE uses purged CV IS accuracy (should be lower than before)
```
