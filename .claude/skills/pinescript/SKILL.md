...
strategy("Name", overlay=true, margin_long=100, margin_short=100,
         ...
         default_qty_value=10000, commission_type=strategy.commission.cash_per_order,
         commission_value=6, pyramiding=1)
```
...
### Stochastic — Raw K, Fast, and Full

Three stochastic variants for different use cases:

```pinescript
// ============================================================================
// RAW K STOCHASTIC — No smoothing (K smooth=1)
// Fastest response, use when you need unsmoothed oscillator
// ============================================================================
stoch_k(int k_len) =>
...
// ============================================================================
// FAST STOCHASTIC — Single SMA smoothing
// ============================================================================
...
// ============================================================================
// FULL STOCHASTIC — Double SMA smoothing
// ============================================================================
...
// ============================================================================
plot(stoch_value, "stoch_value", display=display.none)
```
...
alertcondition(ta.crossover(stoch_value, 80), "Mgmt Long", "Stoch >80 - RAISE STOP")
```
...
```json
{
  ...
  "close": {{close}},
  "stochastics": {
    "fast": {{plot("stoch_fast")}},
    "slow": {{plot("stoch_slow")}}
  }
}
```
...
**WARNING:** `commission.percent` applies to the CASH quantity, NOT the notional value. With leverage, this massively understates real costs.

| Method | Setting | With $500 cash, 20x leverage ($10k notional) |
|--------|---------|----------------------------------------------|
| `commission.percent` | 0.06 | $0.30/side (WRONG — applies to $500) |
| `cash_per_order` | 6 | $6/side (CORRECT — deterministic) |

**Always use `cash_per_order` for leveraged strategies:**
```pinescript
strategy("Name", ...,
    commission_type=strategy.commission.cash_per_order,
    commission_value=6)  // Deterministic, no leverage ambiguity
```

---

## Phantom Trade Bug — CRITICAL

**`strategy.close_all()` + `strategy.entry()` on the same bar creates TWO trades:**
1. Close trade: $0 P&L + commission charged
2. New entry: commission charged again

**This doubles commission on every flip.** In high-frequency strategies, this causes rapid equity drain.

**Fix — auto-reverse with `strategy.entry()`:**
```pinescript
// WRONG — creates phantom trade
strategy.close_all()
strategy.entry("Long", strategy.long)    // 2 trades, 2x commission

// CORRECT — auto-reverses in one trade
strategy.cancel("Exit Short")            // Clear stale exit orders first!
strategy.entry("Long", strategy.long)    // Flips from short→long in 1 order
```

**Always `strategy.cancel()` stale exit orders before flipping.** A pending `strategy.exit()` from the old direction can collide with the new entry and cause unintended exits.

---

## Cooldown Gate Pattern

Prevents rapid-fire flipping that pays commission for nothing. Critical for high-frequency strategies.

```pinescript
i_cooldown = input.int(3, "Min Bars Between Entries", minval=0, maxval=20)

var int entryBar = na    // IMPORTANT: does NOT reset on exit — persists for cooldown
...
bool enter_long = longSignal and isFlat and cooldownOK

// On entry, record bar:
if enter_long
    entryBar := bar_index

// On exit (via strategy.position_size sync):
...
### Commission & Execution (Leveraged Strategies)
- [ ] Commission uses `cash_per_order`, NOT `commission.percent` (leverage-safe)
- [ ] No `strategy.close_all()` for flips — use `strategy.entry()` auto-reverse
- [ ] `strategy.cancel()` clears stale exits before direction flips
- [ ] Cooldown gate applied to ALL entry paths
- [ ] `entryBar` persists through exit (NOT reset in state sync)
...
**v3.0 (2026-02-07) - Skill Separation**
- Removed: Four Pillars strategy-specific content (moved to `four-pillars` skill)
- Added: Raw K stochastic function (`stoch_k()`) as general-purpose variant
- Added: Commission Handling section (general Pine Script platform behavior)
- Added: Phantom Trade Bug section (general Pine Script platform behavior)
- Added: Cooldown Gate Pattern (general strategy pattern)
- Added: Commission & Execution checklist items
- Generalized: All examples use generic variable names
